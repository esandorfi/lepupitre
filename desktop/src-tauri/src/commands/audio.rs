...
pub fn audio_trim_wav(
    ...
    let source_samples = decode_wav_pcm16_mono_16k(&source_bytes)?;
    let (start_idx, end_idx, start_ms, end_ms) =
        resolve_trim_sample_range(source_samples.len(), start_ms, end_ms, TARGET_SAMPLE_RATE)?;
    let trimmed_samples = &source_samples[start_idx..end_idx];
    let trimmed_bytes = encode_wav_pcm16_mono(TARGET_SAMPLE_RATE, 1, trimmed_samples)?;
    let duration_ms = duration_ms_from_sample_count(trimmed_samples.len(), TARGET_SAMPLE_RATE);
...
fn decode_wav_pcm16_mono_16k(bytes: &[u8]) -> Result<Vec<i16>, String> {
    if bytes.len() < 44 {
...
fn duration_ms_from_sample_count(sample_count: usize, sample_rate: u32) -> i64 {
