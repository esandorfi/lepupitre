...
fn set_setting(
    ...
    match value {
        Some(v) => {
            let query =
                format!("INSERT INTO {table} (key, value_json) VALUES (?1, ?2) ON CONFLICT(key) DO UPDATE SET value_json = excluded.value_json");
...
pub fn preference_global_set(
    ...
    let validated_key = validate_key(&key)?;
    validate_value(value.as_deref())?;
    let conn = db::open_global(&app)?;
    set_setting(&conn, "global_settings", validated_key, value.as_deref())
}
...
pub fn preference_profile_set(
    ...
    db::ensure_profile_exists(&app, &profile_id)?;
    let conn = db::open_profile(&app, &profile_id)?;
    set_setting(&conn, "profile_settings", validated_key, value.as_deref())
}
