...
export type QuestSort = "recent" | "az" | "category";
...
function buildRewardState(options: {
  ...
  const nextRewardBadge = computed(() => rewardBadges.value.find((badge) => !badge.unlocked) ?? null);
...
export function createHeroState(params: {
  ...
  const heroQuestRoute = computed(() => (heroQuest.value ? questRoute(heroQuest.value.code) : "/training"));
...
export function createQuestPickerState(params: {
  ...
  const { availableQuests, questPickerSearch, questPickerCategory, questPickerSort, recentAttempts } =
    params;

  const questCategories = computed(() => {
    const categories = Array.from(new Set(availableQuests.value.map((quest) => quest.category))).sort();
  ...
  const pickerVisibleQuests = computed(() => [...recentPickerQuests.value, ...pickerMainQuests.value]);
...
export function createGamificationState(params: {
  t: (key: string) => string;
  uiSettings: Readonly<Ref<{ mascotEnabled: boolean; mascotIntensity: string; gamificationMode: string }>>;
  trainingProgress: Ref<ProgressSnapshot | null>;
  availableQuests: Ref<Quest[]>;
  trainingDailyQuest: Ref<QuestDaily | null>;
  ...
  const dailyLoopSteps = computed<DailyLoopStep[]>(() => {
    ...
    return [
      { id: "practice", title: t("training.daily_loop_step_practice"), done: practicedToday.value, ctaRoute: practiceRoute },
      { id: "feedback", title: t("training.daily_loop_step_feedback"), done: hasFeedbackInRecent.value, ctaRoute: "/feedback" },
      { id: "momentum", title: t("training.daily_loop_step_momentum"), done: (trainingProgress.value?.streak_days ?? 0) >= 3, ctaRoute: "/training" },
    ];
  });
  const dailyLoopCompletedCount = computed(() => dailyLoopSteps.value.filter((step) => step.done).length);
  const dailyLoopIsComplete = computed(() => dailyLoopCompletedCount.value >= dailyLoopSteps.value.length);
...
export function bindHomePageEffects(params: {
  ...
  watch([isQuestPickerOpen, pickerVisibleQuests], () => syncQuestPickerActive(), { deep: false });
  watch(() => locale.value, async () => refreshMascotMessage());
